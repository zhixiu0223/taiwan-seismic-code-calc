...
(40x40cm、2%鋼筋、Pu=147.6kN)算出的 Mn 只有原公式的一半:
...
以內,確認原公式高估強度,方向**不保守**(當初「保守」標籤是錯的)。
...
單一斷面彎矩-曲率分析,並發現原快速公式高估強度(見上方 VL-04/05,
已直接修正)。**尚待**:2 層樓框架完整側推分析、套 FEMA 273 等面積
雙直線法——先完成纖維斷面本身的交叉驗證,才進入 Pushover。
