...
**這不影響 Case-03.5 的位移角 governing 結論**(那個判斷跟彎矩絕對
數值無關)。**但確實影響 Case-03.7、Case-04 的 Design Loop 收斂結果**
——這兩個 Case 的強度檢核都用了同一個被高估的公式,Design Loop
收斂到的 20cm,用修正後的應變相容法重算,強度利用率其實是 159%
(FAIL),真正兩項檢核都通過的最小尺寸是 **25cm**,不是 20cm。
詳見 Case-03.6c(修正版強度檢核,不修改 Case-03.6/03.7/04 本身)。

**受影響範圍精確盤點**(逐一檢查各 Case 的建置腳本,不是憑印象):

| Case | 是否用到問題公式 | 結論是否翻盤 |
|---|---|---|
| Case-03.5 | 否(只檢核位移角) | 不受影響 |
| Case-03.6 | 是(問題來源) | 已知限制,不修改 |
| Case-03.6b | 是 | 否(示範用40cm修正後仍PASS,15.8%) |
| **Case-03.7** | 是 | **是,20cm→25cm** |
| **Case-04** | 是 | **是,20cm→25cm** |
| Case-04.5 | 否(核心是位移/勁度比較) | 不受影響 |
| Case-05 | 否(核心是位移比較,用40cm驗證斷面本就PASS) | 不受影響 |
...
| VL-05 | Design Loop真正governing尺寸 | 修正版強度檢核 vs 原公式 | 20cm→25cm, 高估比例隨尺寸變動(15cm時達3.5倍) | Case-03.6c |
